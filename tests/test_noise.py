...
def test_stderr_rejects_tiny_samples():
    with pytest.raises(ValueError, match="greater than 3"):
        correlation_stderr(np.array(0.5), 3)
...
def test_diagonal_is_not_an_estimate():
    r = np.array([[1.0, 0.4], [0.4, 1.0]])
    _, high = correlation_interval(r, n_obs=200)
    assert high[0, 0] == pytest.approx(1.0, abs=1e-5)
